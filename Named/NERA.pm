...
sub detectentities
{
	...
	# If the $word contains month specification as 'mon' format, this flag sets.
	if($word =~ m/(^|[^A-Za-z])(($jan|$feb|$mar|$apr|$may|$jun|$jul|$aug|$sep|$oct|$nov|$dec)\.?)([^A-Za-z\.]|$)/g)
	{
		$flag1 = 1;
	}
	...
	#If the $word contains week specification in text format this flag sets.
	if( $word =~ m/(^|[^A-Za-z])(($sun|$mon|$tue|$wed|$thu|$fri|$sat)\.?)([^A-Za-z\.]|$)/g)
	{
		$flag2 = 1;
	}
	...
	if($word =~ m/(^|[^0-9A-Za-z])($num_y|$four_digit_year)($dateseperator|\.)($num_m|$Month)($dateseperator|\.)($num_day)([^0-9A-Za-z]|$)/g)
	{
		$flag3 = 1;
	}# To know whether $word has above formats of date.
	...
	#To know whether $word has above formats of date.
	if($word =~ m/(^|[^0-9A-Za-z])($num_day)($dateseperator|\.)($num_m|$Month)($dateseperator|\.)($num_y|$four_digit_year)([^0-9A-Za-z]|$)/g)
	{
		$flag4 = 1;
	}
	...
	#To know whether $word has above formats of date.
	if($word =~ m/(^|[^0-9A-Za-z])($num_m|$Month)($dateseperator|\.)($num_day)($dateseperator|\.)($num_y|$four_digit_year)([^0-9A-Za-z]|$)/g)
	{
		$flag5 = 1;
	}
	...
	#To know whether $word has above formats of date.
	if($word =~ m/(^|[^A-Za-z0-9])($num_day)([ ]*)($Month)([, ]*)($four_digit_year|[03-9][0-9])([^0-9A-Za-z]|$)/g)
	{
		$flag6 = 1;
	}
	...
	#To know whether $word has above formats of date.
	if($word =~ m/(^|[^A-Za-z0-9])($Month)([ ]*)($num_day)([, ]*)($four_digit_year)([^0-9A-Za-z]|$)/g)
	{
		$flag7 = 1;
	}
	...
	#Detecting Time...
	# $pod --- Part of the day....
	# AMs and PMs are there on $word
	if($word =~ m/(^|[^A-Za-z])($am|$pm)([^A-Za-z]|$)/g)
	{
		$flag10 = 1;
	}
	#detecting A.M am
	...
	#Cover Zonediff +0530
	if($word =~ m/(==time[0-9:,]{6,15}=)([ :\,\t]*)?($zonediff)([^0-9]|$)/g)
	{
		$flag11 = 1;
	}
	...
	#print "$word\n";
	$word = $flag1.$flag2.$flag3.$flag4.$flag5.$flag6.$flag7.$flag8.$flag9.$flag10.$flag11." ".$word;
	return ($word);
